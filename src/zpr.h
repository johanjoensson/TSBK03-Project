...
static void updateCameraMatrix(mat4 *matrix);    // matrix is passed as a parameter when both cameraMatrix
                                                 // and camera and lookAtPoint have to be transformed.
                                                 //  if matrix = NULL, only cameraMatrix is updated

static void zprMouse(int button, int state, int x, int y);  // records mouse buttons' state
static void zprKey(unsigned char key, int x, int y); // camera control with keys. Updates viewMatrix
